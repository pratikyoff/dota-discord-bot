using Bot.Universal;

namespace Bot
{
    public static class BotDetails
    {
        public static string Name { get => "dota-stalker-bot"; }
        public static string ClientId { get => "356727461385601025"; }
        public static string ClientSecret { get => "H8Ny3LX0VI24vjo_EL2_u85YCYAYFVBl"; }
        public static string Username { get => "dota-stalker-bot#3927"; }
        public static string Token { get => FileOperations.ReadLineNo("token.txt", 0); }
        public static string ApiUrl { get => "https://discordapp.com/api/"; }
        public static string CommandPrefix { get => "!"; }
        public static ulong BotFeedChannel { get => 387244251936849930; }
        public static ulong BotDumpChannel { get => 390087493766217738; }
    }
}
